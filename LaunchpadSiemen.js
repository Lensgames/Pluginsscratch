...
/* -------------------------------------------------------------------------	*/
	...
	function failure(error) {
		alert( "Midi Not Connected, Connect And try again.");
	}
...
/* -------------------------------------------------------------------------	*/
	...
	var descriptor = {
		blocks: [
			[' ', 'LED %n %n %n', 'midiout', 1, 66, 120],
			['h', 'Channel Button Pressed', 		's_getcc'],
			['h', 'When Key pressed',	's_getnoteon'],
			['h', 'Key %n Pressed?',		's_getkeyonnum',60],
			['h', 'Key pressed in?',			's_getkeyon'],
			['h', 'Key Released?',		's_getkeyoff'],
			['r', 'Pressed Key', 			's_note'],
			['r', 'Touch?', 			's_vel'],
		...
		menus: {
			midiEvent: ['key ON', 'key off', 'Channel Button', 'p-bend', 'pg-chg'],
		},

	};

    // Register the extension
    ScratchExtensions.register('Launchpad Mini', descriptor, ext);
})({});